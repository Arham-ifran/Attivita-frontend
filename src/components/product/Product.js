	...
	return (
		<section className="products-section custom-container home-product-sec">
			<Container fluid>
				...
				<Tab.Container defaultActiveKey="0-event">
							...
							<Nav variant="pills" className="row flex-row">
								{
									...
									manufacturers.map((manufacturer, index) => {
										return (
											<Nav.Item className="col-xl-2 col-lg-4 col-md-4 col-sm-4" key={index}>
											...
											<Row>
												{
													...
													manufacturer.products.map((product, index_) => {
														...
														if(window.location.pathname.split('/')[1] == '' && index_ > 3 )
														{
															allow_display = false
														}
														if(allow_display){
...
export default connect(mapStateToProps, { beforeProducts, getProductsList, getVATforCountry })(Product);